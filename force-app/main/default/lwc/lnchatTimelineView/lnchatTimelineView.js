...
export default class LnchatTimelineView extends LightningElement {
    ...
    set events(value) {
        ...
        this.processedEvents = sorted.map((event, idx) => ({
            ...
            iconClass: 'timeline-icon ' + (TYPE_ICON_COLORS[event.type] || TYPE_ICON_COLORS.default),
            statusClass: 'event-status ' + (STATUS_COLORS[event.status] || STATUS_COLORS.pending),
            containerClass: 'timeline-event' + (idx % 2 === 0 ? '' : ' timeline-event-alt'),
