...
export default class LnchatResponseRenderer extends NavigationMixin(
  LightningElement
) {
  ...
  get isKpiCards() {
    return this.responseType === "kpi_cards";
  }
  get isChart() {
    return this.responseType === "chart";
  }
  get isComparison() {
    return this.responseType === "comparison";
  }
  get isTimeline() {
    return this.responseType === "timeline";
  }
  get isAlert() {
    return this.responseType === "alert";
  }
  get isTable() {
    return this.responseType === "table";
  }
  get isRecordSummary() {
    return this.responseType === "record_summary";
  }
  get showTimeline() {
    return (
      this.isRecordSummary &&
      this.objectApiName === "Account" &&
      !!this.recordId
    );
  }
  get isRecordAction() {
    return this.responseType === "record_action";
  }
  get isRecordList() {
    return this.responseType === "record_list";
  }
  get isSummary() {
    return (
      (this.responseType === "summary" || !this.responseType) &&
      !this.isRecordAction &&
      !this.isRecordSummary &&
      !this.isTable &&
      !this.isRecordList
  ...
  get hasTitle() {
    return !!this.title && !this.isSummary && !this.isChart;
  }
  get hasActions() {
  ...
  get chartType() {
    return (this.data && this.data.chartType) || "bar";
  }
  get chartTitle() {
    return this.title;
  }
  ...
  get alertSeverity() {
    return (this.data && this.data.severity) || "info";
  }
  get alertTitle() {
    return (this.data && this.data.alertTitle) || this.title;
  }
  get alertMessage() {
  ...
  get alertAllowBypass() {
    return !!(this.data && this.data.allowBypass);
  }
  ...
  get recordSummaryData() {
    return this.data || {};
  }

  // ─── Record Action ───────────────────────────────────────────────────────

  get recordActionOperation() {
    return (this.data && this.data.operation) || "create";
  }
  get recordActionObject() {
    return (this.data && this.data.objectApiName) || "";
  }
  get recordActionRecordId() {
    return (this.data && this.data.recordId) || "";
  }
  get recordActionFields() {
  ...
  handleAction(event) {
    ...
    const inputVarsList = Object.entries(inputVariables).map(
      ([name, value]) => ({
        name,
    ...
    this.dispatchEvent(
      new CustomEvent("actionlaunched", {
