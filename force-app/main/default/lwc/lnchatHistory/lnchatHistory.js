...
export default class LnchatHistory extends LightningElement {
  ...
  addMessage(role, content, responseType, attachmentName) {
    ...
    const message = {
      ...
      isUser: role === "user",
      wrapClass:
        "message-wrap " + (role === "user" ? "user-wrap" : "assistant-wrap"),
      ...
      intentBadgeClass:
        INTENT_BADGE_CLASS[parsedResponseType] || "intent-badge badge-grey",
  ...
  buildEmailHtml(msg) {
    ...
    const ths =
      "background:#f3f4f5;padding:8px;text-align:left;font-size:12px;color:#706e6b;border-bottom:2px solid #dddbda;";
    const tds =
      "padding:8px;border-bottom:1px solid #f3f4f5;font-size:13px;color:#032D60;";
    const foot =
      '<hr style="border:none;border-top:1px solid #dddbda;margin:20px 0 10px"/><p style="font-size:11px;color:#aaa;margin:0">Sent from Salesforce AI Assistant</p>';
    ...
    if (summary)
      b += `<p style="font-size:14px;color:#444;margin:0 0 16px;line-height:1.5">${this.esc(summary)}</p>`;
    ...
    if (type === "record_summary") {
      ...
      if (hdr.subtitle)
        b += `<p style="font-size:13px;color:#706e6b;margin:0 0 12px">${this.esc(hdr.subtitle)}</p>`;
      if (kpis.length) {
        b +=
          '<p style="font-size:12px;color:#706e6b;font-weight:600;margin:0 0 4px">KEY METRICS</p><div style="margin:0 0 16px">';
        kpis.forEach((k) => {
      ...
      if (d.details && d.details.length) {
        ...
        d.details.forEach((det) => {
          b += `<li style="font-size:12px;color:#333;margin:2px 0">${this.esc(det)}</li>`;
        });
        b += "</ul>";
      }
      b += "</div>";
      ...
      (d.sections || []).forEach((s) => {
        if (s.heading)
          b += `<h3 style="font-size:14px;color:#032D60;margin:12px 0 4px">${this.esc(s.heading)}</h3>`;
  ...
  buildRecordListEmailHtml(msg, result) {
    ...
    b += `<h2 style="font-size:20px;margin:0 0 8px;color:#032D60">${this.esc(title)}</h2>`;
    if (summary)
      b += `<p style="font-size:14px;color:#444;margin:0 0 8px;line-height:1.5">${this.esc(summary)}</p>`;
    ...
    if (rows.length) {
      ...
      b +=
        '<p style="font-size:13px;color:#706e6b;font-style:italic">No records found.</p>';
    }
