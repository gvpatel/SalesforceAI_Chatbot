...
export default class LnchatChartWidget extends LightningElement {
    ...
    renderChart() {
        ...
        const multiColor = this.chartType === 'donut' || (this.chartType === 'bar' && series.length === 1);

        const mappedDatasets = series.map((ds, idx) => {
            const color = ds.color || SF_PALETTE[idx % SF_PALETTE.length];
            const bgColors = multiColor
                ? labels.map((_, i) => this.hexToRgba(SF_PALETTE[i % SF_PALETTE.length], this.chartType === 'donut' ? 1 : 0.82))
                : this.chartType === 'line'
        ...
        const config = {
            ...
            options: {
                ...
                scales: this.chartType !== 'donut' ? {
                    x: {
                        grid: { display: false },
                        ticks: {
                            font: { size: 10, family: 'Salesforce Sans, Arial, sans-serif' },
                            color: '#706e6b',
                            maxRotation: 45
                        }
                    },
